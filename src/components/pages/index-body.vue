    ...
    <el-row :gutter="20" style="margin-top: 30px;">
      <el-col :span="24">
        ...
        <el-row style="margin-top: 50px;" v-for="(obj1, index1) in getRowNums(data2)" :key="getRandomString(20)">
          <el-col :span="4" v-for="(obj2, index2) in mySlice(data2, index1 * 4, (index1 + 1) * 4)"
            :key="getRandomString(21)" :offset="index2 > 0 ? 2 : 1">
            <el-card :body-style="{ padding: '0px', textAlign: 'center' }">
              <img :src="obj2.picture" class="image" style="width: 100%" alt="请检查网络连接">
              <div style="padding: 10px;">
...
export default {
  ...
  methods: {
    ...
    clickGoodsInfoButton(goodsId) {
      ...
      let requestUrl = "http://localhost:8080/api/goods/" + goodsId;
      console.log(goodsId);
      $.get(requestUrl, function (data) {
        self.goodsInfoName = data.data.title;
        self.goodsInfoPrice = data.data.price;
        self.goodsInfoDscrip = data.data.detail;
        $.get("http://localhost:8080/api/goods/" + goodsId   + "/imgmap", jsonObj, function (data) {
          console.log(data);
          self.goodsInfoImg = data.data;
        }, "json");
      }, "json");
    },
    //点击加入购物车按钮
